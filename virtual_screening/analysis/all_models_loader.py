...
def stage_1_results(model_directory, data_directory):
    ...
    stage_1_dict = {'random_forest' : get_rf_results(class_dirs[0], data_directory),
                    'irv' : get_irv_results(class_dirs[1], data_directory),
                    'light_chem' : get_lightchem_results(class_dirs[2], data_directory),
                    'neural_networks' : get_nn_results(class_dirs[3], data_directory),
                    'docking' : get_docking_results(class_dirs[4], data_directory)
                     }    
    return stage_1_dict           
...
model_name -> fold_# -> labels, y_train, y_val, y_test, y_pred_on_train, y_pred_on_val, y_pred_on_test
"""
def get_docking_results(model_directory, data_directory, k=5):
    model_list = {}
    if not os.path.exists(model_directory):
        return model_list
    ...
    pria_df = pd.read_csv(model_directory+'/lc123_all_docking_scores.csv')
    rmi_df = pd.read_csv(model_directory+'/lc123_rmi_all_docking_scores.csv')
